...
class EventController extends Controller
{
    ...
    public function filters(): JsonResponse
    {
        ...
        $options = Cache::remember('events:filters:v1', now()->addMinutes(10), function () {
            ...
            // min()/max() on the indexed created_time are index-served edge reads.
            $lo = Event::query()->where('is_public', true)->min('created_time');
            $hi = Event::query()->where('is_public', true)->max('created_time');

            return [
                'cities' => $cities,
                'categories' => Event::CATEGORIES,
                'date_min' => $lo ? date('Y-m-d', (int) $lo) : null,
                'date_max' => $hi ? date('Y-m-d', (int) $hi) : null,
            ];
        });
