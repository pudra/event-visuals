...
renders it in **the viewer's own timezone** via `Intl.DateTimeFormat`, formatted **US-style**
(`en-US`, e.g. "Jun 18, 2026, 10:53 AM"), with the gallery header showing the active timezone
(e.g. "EDT"). Emails state times explicitly in UTC. The native date-filter input follows the
viewer's locale, so a US user sees `MM/DD/YYYY`; it stays native rather than a custom widget
so it remains keyboard- and screen-reader-complete.
...
I built this with Claude Code, working spec-first. Before any code I had it read the whole
starter kit, so the solution follows the existing Inertia/Vue conventions instead of fighting
them. The decisions that mattered were mine: denormalizing for scale, the `is_public` index
trick, the offline geocoding strategy, and the two layouts. I used the agent to execute them
quickly, then reviewed every file; I am accountable for what ships regardless of who drafted
the first pass.

Two things that show I run and verify rather than just generate:

- The starter's own seeder inserts 4,000 rows per statement, which is 40,000 SQLite bind
  variables, over the 32,766 limit, so the default `db:seed` fails out of the box. I caught it
  only by running the full 1.25M-row seed, then reduced the chunk size.
- The register endpoint was redirecting on validation failure instead of returning JSON 422
  (the Inertia/web stack does that), which would have broken the dialog's error handling. I
  made it return JSON explicitly and added a test that locks it in; the date filter uses the
  same pattern.

Tests and static analysis (PHPStan level 7) were part of the loop, not an afterthought; both
